;(function($) {
	...
	window[fnName] = function(setConfig) {
		...
		var cover = $(_this.cover), title = $(_this.title), thisTime = $(_this.timeView.thisTime), countTime = $(_this.timeView.countTime), thisSetbacks = $(_this.setbacks.thisSetbacks), cacheSetbacks = $(_this.setbacks.cacheSetbacks), setbacks = $(_this.setbacks.setbacks), volumeCircular = $(_this.setbacks.volumeCircular), volumeSetbacks = $(_this.setbacks.volumeSetbacks), volumeBox = $(_this.volume.volumeBox), play = $(_this.button.play), prev = $(_this.button.prev), next = $(_this.button.next), menuBtn = $(_this.button.menu), volume = $(_this.button.volume), menuClose = $(_this.button.menuClose), backs = $(_this.button.backs);
		...
		_this.createAudio = function() {
			...
			_this.volumeSet();
			var nTitle = song[songEq].title;
			var nAntor = song[songEq].antor;
			var nNlbum = song[songEq].album;
			if (nTitle.length > 0) {
				var titleStr = song[songEq].title + " ";
			} else {
				var titleStr = "";
			}
			
			if (nAntor && nAntor.length > 0) {
				var antorStr = "              作者: " + song[songEq].antor + "                ";
			} else {
				var antorStr = "";
			}
			
			if (nNlbum && nNlbum.length > 0) {
				var albumStr = "           专辑: " + song[songEq].album + "               ";
			} else {
				var albumStr = "";
			}
			var newTitle = titleStr + antorStr + albumStr;
			title.text(newTitle || '未知歌曲');

			$('.audio-auto-title').marquee({
		...
		_this.upMenu = function() {
			...
			var song = _this.song, inline = $(_this.menu.list).empty();
		...
		prev.on('click', function() {--songEq;
			songEq = (songEq >= 0) ? songEq : _this.song.length - 1;
			_this.selectMenu(songEq, true);
		});

		next.on('click', function() {++songEq;
		thisSetbacks.css({
							'width' : "0%"
						});
			songEq = (songEq < _this.song.length) ? songEq : 0;
